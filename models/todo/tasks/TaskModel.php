...
class TaskModel
{
    ...
    {
        $query = "INSERT INTO todo_list (user_id, title, description, category_id, status, priority, finish_date, reminder_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?)";

        try {
            $stmt = $this->db->prepare($query);
            $stmt->execute([$data['user_id'], $data['title'], $data['description'], $data['category_id'], $data['status'], $data['priority'], $data['finish_date'], $data['reminder_at']]);
            return true;
        } catch (\PDOException $e) {
            return false;
        }
